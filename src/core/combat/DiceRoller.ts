...
export class DiceRoller {
   ...
   */
  static calculateDamage(formula: string, modifier: number, isCritical: boolean = false): {
    rolls: number[];
    modifier: number;
    total: number;
    isCritical: boolean;
   ...
   * @returns Save result
   */
  static rollSavingThrow(abilityModifier: number, proficiencyBonus: number = 0): number {
   ...
   */
  static rollAbilityCheck(abilityModifier: number, proficiencyBonus: number = 0): number {
    return this.rollD20() + abilityModifier + proficiencyBonus;
  }
