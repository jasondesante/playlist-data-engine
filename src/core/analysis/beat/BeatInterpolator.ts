...
export class BeatInterpolator {
    ...
    /**
     * Approach 1: Histogram-Based Fixed Grid
     *
     * Simple approach:
     * 1. Use quarter note as fixed grid spacing
     * 2. Generate rigid grid from first detected beat
     * 3. No tempo drift handling
     *
     * @param beatMap - Original beat map
     * @param quarterNote - Quarter note detection
     * @returns Grid beats with source information
     */
    private interpolateHistogramGrid(
        beatMap: BeatMap,
        quarterNote: QuarterNoteDetection
    ): BeatWithSource[] {
        ...
        // Build map of detected beats by timestamp
        ...
        const firstBeat = beats[0].timestamp;
        ...
        // Extrapolate before first beat if enabled
        if (this.options.extrapolateStart) {
            let time = firstBeat - qn;
            while (time >= 0) {
                const confidence = this.calculateConfidence(
                    gridWeight,
                    anchorWeight * 0.5, // Lower anchor confidence for extrapolation
        ...
        // Generate grid through the audio
        let time = firstBeat;
        let beatIndex = 0;

        while (time <= lastBeat) {
            // Check for detected beat at this position
            const detected = this.findBeatNear(detectedByTime, time, tolerance);

            if (detected) {
                // Use detected beat
                gridBeats.push(this.createDetectedBeat(detected));
                beatIndex = beats.indexOf(detected);
            } else {
                // Create interpolated beat
                const nearestAnchor = this.findNearestAnchor(beats, time, beatIndex);
                const confidence = this.calculateConfidence(
                    gridWeight,
                    anchorWeight * nearestAnchor.confidence,
                    paceWeight * quarterNote.confidence
                );
                gridBeats.push(this.createInterpolatedBeat(time, confidence, nearestAnchor.timestamp));
            }

            time += qn;
        }

        // Extrapolate after last beat if enabled
     ...
     * 1. Enhanced quarter note detection with KDE
     * 2. Grid with distributed error correction at anchors
     * 3. Confidence scoring based on distance and alignment
     *
     * @param beatMap - Original beat map
     * @param quarterNote - Quarter note detection
     * @param gapAnalysis - Gap analysis result
    ...
    private interpolateDualPass(
        ...
        if (this.options.extrapolateStart) {
            ...
            while (time >= 0) {
                const confidence = this.calculateConfidence(
                    gridWeight * gapAnalysis.gridAlignmentScore,
                    anchorWeight * 0.5,
                    paceWeight * quarterNote.confidence
                );
                gridBeats.unshift(this.createInterpolatedBeat(time, confidence, beats[0].timestamp));
        ...
        // Pass 1: Generate grid with error correction at each anchor
        // Calculate cumulative error at each detected beat
        const anchorErrors = new Map<number, number>();
        let expectedPosition = beats[0].timestamp;

        for (const beat of beats) {
            const error = beat.timestamp - expectedPosition;
            anchorErrors.set(beat.timestamp, error);
            // Update expected position for next beat
            const beatsToNext = Math.round((beat.timestamp - (beats.indexOf(beat) > 0 ? beats[beats.indexOf(beat) - 1].timestamp : beat.timestamp)) / qn);
            expectedPosition = beat.timestamp + (Math.max(1, beatsToNext) * qn);
        }

        // Pass 2: Generate interpolated beats with error distribution
        for (let i = 0; i < beats.length; i++) {
            const anchor = beats[i];
            const anchorError = anchorErrors.get(anchor.timestamp) || 0;

            // Add the detected beat
            gridBeats.push(this.createDetectedBeat(anchor));

            // If there's a next beat, interpolate with error distribution
            if (i < beats.length - 1) {
                const nextAnchor = beats[i + 1];
                const nextError = anchorErrors.get(nextAnchor.timestamp) || 0;
                const gapDuration = nextAnchor.timestamp - anchor.timestamp;

                // Calculate beats to interpolate
                const expectedBeats = Math.round(gapDuration / qn);

                if (expectedBeats > 1) {
                    // Distribute error across interpolated beats
                    const errorPerBeat = (nextError - anchorError) / expectedBeats;

                    for (let j = 1; j < expectedBeats; j++) {
                        // Apply error correction
                        const correctedPosition = anchor.timestamp + (j * qn) + (errorPerBeat * j);
                        const time = correctedPosition;

                        // Skip if too close to next anchor
                        if (time >= nextAnchor.timestamp - tolerance) break;

                        // Calculate confidence with dual-pass scoring
                        // Include grid alignment score in confidence
                        const distanceToNearestAnchor = Math.min(
                            time - anchor.timestamp,
                            nextAnchor.timestamp - time
                        );
                        const anchorProximityFactor = Math.min(1, distanceToNearestAnchor / qn);

                        const confidence = this.calculateConfidence(
                            gridWeight * gapAnalysis.gridAlignmentScore,
                            anchorWeight * ((anchor.confidence + nextAnchor.confidence) / 2) * (1 - anchorProximityFactor * 0.2),
                            paceWeight * quarterNote.confidence
                        );
        ...
        // Extrapolate after last beat
        if (this.options.extrapolateEnd) {
            let time = beats[beats.length - 1].timestamp + qn;
            while (time <= duration) {
                const confidence = this.calculateConfidence(
                    gridWeight * gapAnalysis.gridAlignmentScore,
                    anchorWeight * 0.5,
